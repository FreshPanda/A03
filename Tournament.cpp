...
string Tournament::print_scores() {
    ...
    for(k=0; k<Round1.size()/2; k++) // there will always be an amount of fights equal to half the members in the round vector
    {
        for(j = 0; j < 5; j++)
        {
            //Round1.push_back(Round1.at(i));
            fight_winner = newRef.evaluate(Round1.at(i), Round1.at(i+1)); // Finding the fight winners and storing in the string
            if(fight_winner == "p1")
            {
    ...
    {
        Round2.push_back(k)->setCounter(0);
    }
    //
    ...
    for(k=0;k<Round2.size()/2;k++) // There will always be an amount of fights equal to half the members in the round vector
    {
        ...
        {
            fight_winner = newRef.evaluate(Round2.at(i), Round2.at(i+1)); // Finding the fight winners and storing in the string
            if(fight_winner == "p1")
            {
                p1wins++; // Iterating p1 wins
            }
    ...
    {
        Round3.at(k)->setCounter(0);
    }

    //ROUND 3
    for(j = 0; j < 5; j++) // Playing 5 fights
    {
        fight_winner = newRef.evaluate(Round3.at(0), Round3.at(1)); // Finding the fight winners and storing in the string
        if(fight_winner == "p1")
        {
            p1wins++; // Iterating p1 wins
        }
